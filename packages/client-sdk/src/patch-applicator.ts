...
export class PatchApplicator {
  ...
  private readonly backupExtension: string;
   ...
   */
  async applyPatches(patches: AnnotationPatch[]): Promise<PatchResult[]> {
    const results: PatchResult[] = [];

    // Group patches by file for efficient processing
    const patchesByFile = new Map<string, AnnotationPatch[]>();
   ...
   */
  async previewPatches(patches: AnnotationPatch[]): Promise<PatchResult[]> {
    const results: PatchResult[] = [];
   ...
   * @returns Result
   */
  async applyPatch(patch: AnnotationPatch): Promise<PatchResult> {
   ...
   */
  async validatePatches(patches: AnnotationPatch[]): Promise<string[]> {
    const errors: string[] = [];
    ...
    for (const patch of patches) {
      ...
      if (patch.lineNumber < 1 || patch.lineNumber > lines.length + 1) {
        errors.push(
          `Invalid line number ${patch.lineNumber} for ${patch.filePath} (file has ${lines.length} lines)`,
        );
      }
   ...
   */
  private async applyPatchesToFile(
    filePath: string,
   ...
   */
  private async previewPatchesToFile(
    filePath: string,
    patches: AnnotationPatch[],
  ): Promise<string> {
    const fullPath = path.join(this.projectRoot, filePath);
   ...
   */
  async restoreFromBackups(filePaths: string[]): Promise<PatchResult[]> {
    const results: PatchResult[] = [];
   ...
   */
  async cleanupBackups(filePaths: string[]): Promise<void> {
    for (const filePath of filePaths) {
